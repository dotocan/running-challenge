...
export const GET: APIRoute = async ({ request, redirect }) => {
  ...
  const clientSecret =
    import.meta.env.STRAVA_CLIENT_SECRET || process.env.STRAVA_CLIENT_SECRET;
  const allowedId =
    import.meta.env.ALLOWED_STRAVA_ID || process.env.ALLOWED_STRAVA_ID;
  ...
  try {
    ...
    if (!tokenResponse.ok) {
      return new Response(`Strava Error: ${JSON.stringify(data)}`, {
